...
_DEFAULT_MAX_NEW_TOKENS: int = 384
_DEFAULT_TEMPERATURE: float = 0.1
_DEFAULT_TOP_P: float = 0.85
_DEFAULT_TOP_K: int = 40
_DEFAULT_REPETITION_PENALTY: float = 1.15
_DEFAULT_NO_REPEAT_NGRAM_SIZE: int = 5
...
class LocalInferenceProvider(BaseLLMProvider):
    ...
    device : str | None
        ...
        (MPS → CUDA → CPU).
    max_new_tokens : int
        Maximum number of tokens to generate.  Default: 384.
    temperature : float
        Sampling temperature.  Default: 0.1.
    top_p : float
        Nucleus sampling probability.  Default: 0.85.
    top_k : int
        Top-k sampling parameter.  Default: 40.
    repetition_penalty : float
        Repetition penalty factor.  Default: 1.15.
    no_repeat_ngram_size : int
        Size of n-grams that cannot be repeated. Default: 5.
    ...
    def __init__(
        ...
        no_repeat_ngram_size: int = _DEFAULT_NO_REPEAT_NGRAM_SIZE,
    ) -> None:
        ...
        logger.info(
            ...
            "max_new_tokens=%d, temperature=%.2f, top_p=%.2f, top_k=%d, "
            "repetition_penalty=%.2f, no_repeat_ngram_size=%d).  Model will be loaded on first use.",
    ...
    def _run_inference(self, inputs: Any) -> Any:
        ...
        # Determine stop tokens dynamically to include both standard EOS and Qwen's turn-end token.
        eos_token_ids = [self._tokenizer.eos_token_id]
        im_end_id = self._tokenizer.convert_tokens_to_ids("<|im_end|>")
        if im_end_id is not None and im_end_id != self._tokenizer.unk_token_id:
            eos_token_ids.append(im_end_id)

        # Pad token handling: fall back to eos_token_id if pad_token_id is not configured/unknown.
        pad_token_id = self._tokenizer.pad_token_id
        if pad_token_id is None or pad_token_id == self._tokenizer.unk_token_id:
            pad_token_id = self._tokenizer.eos_token_id

        # Greedy vs Sampling logic based on temperature.
